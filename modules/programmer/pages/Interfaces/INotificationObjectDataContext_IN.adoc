= INotificationObjectDataContext -- интерфейс

Интерфейс контекста данных объекта. Нужен для выгрузки данных объекта в произвольном формате (который понимает следующий компонент по обработке уведомления).
