...
|`CreateComponent<T>(string, ObjectContext, string)`
a|Создать компонент определённого `TypeName`, инициализировать настройками `context` и `settings` (если поддерживается `IInitializeWithObjectContext`, `IInitializeWithSettings`) и возвратить `T`

.Параметры:
* `typeName` -- Полное имя класса
